...
// One line per refresh run; over months this becomes the dataset for
// "when should we fly / book from here" trend analysis.
...
const nightsOk = (dep, ret) => {
  const n = Math.round((new Date(ret) - new Date(dep)) / 86400000);
  return n >= cfg.nights.min && n <= cfg.nights.max;
};

const snapshot = { ts: new Date().toISOString(), dests: {} };
for (const dest of cfg.destinations) {
  ...
  if (cash && !cash.sample) {
    ...
    for (const en of cash.entries) {
      if (en.dest !== dest || !en.price || !nightsOk(en.dep, en.ret)) continue;
      if (!cheapest || en.price < cheapest) cheapest = en.price;
    }
    if (cheapest) row.c = cheapest;
  }
  if (Object.keys(row).length) snapshot.dests[dest] = row;
}

fs.appendFileSync("data/history.ndjson", JSON.stringify(snapshot) + "\n");
console.log(`Appended history snapshot covering ${Object.keys(snapshot.dests).length} destinations.`);
